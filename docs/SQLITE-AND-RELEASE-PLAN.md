...
NOT IMPLEMENTED: generated_data_services.scriban still calls UseSqlServer unconditionally. Add Database:Provider (SqlServer default / Sqlite opt-in / invalid rejected), provider-specific registration, SQL Server-only retries and query splitting, portable UTC DateTimeOffset mapping and decimal query warnings. Update setup docs. Test default, explicit SQL Server, SQLite and invalid-provider output, and compile generated registrations with both providers. Existing 25 tests pass. MSBuild stays at 18.9.6 because 18.10.1 targets net11.0.
